-- =====================================================
-- NAPLET - CARTEIRA DE DOCUMENTOS DO BEBE
...
CREATE INDEX IF NOT EXISTS idx_document_types_order ON public.document_types(order_index);
...
CREATE INDEX IF NOT EXISTS idx_baby_documents_expiration ON public.baby_documents(expiration_date);
...
CREATE POLICY "baby_documents_select" ON public.baby_documents
    FOR SELECT USING (
        ...
        OR baby_id IN (SELECT baby_id FROM public.caregivers WHERE user_id = auth.uid() AND accepted_at IS NOT NULL)
    );
...
-- =====================================================
-- 4. CRIAR TABELA DE ARQUIVOS/PAGINAS DO DOCUMENTO
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_document_files_page ON public.document_files(document_id, page_number);

-- RLS
ALTER TABLE public.document_files ENABLE ROW LEVEL SECURITY;

-- Politicas (herdam do documento pai)
...
CREATE POLICY "document_files_select" ON public.document_files
    FOR SELECT USING (
        document_id IN (
            SELECT bd.id FROM public.baby_documents bd
            WHERE bd.baby_id IN (SELECT id FROM public.babies WHERE owner_id = auth.uid())
            OR bd.baby_id IN (SELECT baby_id FROM public.caregivers WHERE user_id = auth.uid() AND accepted_at IS NOT NULL)
        )
    );
...
-- =====================================================
-- 5. CRIAR BUCKET NO STORAGE PARA DOCUMENTOS
-- =====================================================

-- Criar bucket (execute separadamente se necessario)
...
SELECT 'document_types' as table_name, COUNT(*) as count FROM public.document_types
UNION ALL
SELECT 'baby_documents' as table_name, COUNT(*) as count FROM public.baby_documents
UNION ALL
SELECT 'document_files' as table_name, COUNT(*) as count FROM public.document_files;

-- Listar tipos de documentos
SELECT code, name, icon, color, has_expiration, order_index
...
SELECT 'Carteira de Documentos configurada com sucesso!' as status;
